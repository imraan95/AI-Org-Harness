"""T055: POST /knowledge/{id}/approve, seeding fixtures through whichever
...
def _fixture_record(*, status: str) -> KnowledgeRecord:
    ...
    return KnowledgeRecord(
        id=f"K-t055-{uuid.uuid4()}",
        type="fact",
        topic=f"t055_{uuid.uuid4().hex[:8]}",
        statement="Fixture statement for T055's approve test.",
        status=status,
        confidence=0.75,
...
async def test_approve_flips_a_pending_review_record_to_active():
    ...
    response = client.post(f"/knowledge/{record.id}/approve", headers=_AUTH_HEADERS)

    assert response.status_code == 200
    assert response.json()["status"] == "active"


def test_approve_returns_404_for_unknown_id():
    client = TestClient(app)
    response = client.post(
        f"/knowledge/K-does-not-exist-{uuid.uuid4()}/approve", headers=_AUTH_HEADERS
    )

    assert response.status_code == 404


def test_approve_returns_401_without_a_token_or_service_key():
    client = TestClient(app)
    response = client.post(f"/knowledge/K-does-not-exist-{uuid.uuid4()}/approve")
