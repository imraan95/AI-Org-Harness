...
**Do:** By user decision - the 12 built-in `KnowledgeType` values (decision, fact, customer_insight, strategy, product_requirement, process, policy, person, ownership, action, hypothesis, conflict) stay fixed, since every MCP tool (`get_recent_decisions`, `get_customer_insights`, etc.) and the pipeline's classifier depend on their exact values. Users can additionally define their own custom types on top of that fixed set. Concretely: a new `custom_knowledge_types` table (Supabase migration, `libs/db`); new `harness-api` endpoints (`GET /taxonomy/types` - built-ins plus an org's custom ones; `POST /taxonomy/types` to add one; `DELETE /taxonomy/types/{id}` - built-ins can't be deleted); loosen `KnowledgeRecord.type` from the strict `KnowledgeType` enum to `str` (validated against built-in ∪ custom at the API boundary, since custom types are dynamic/DB-backed and the model itself has no way to know about them); a small taxonomy-management section in `apps/web` (list/add/delete custom types); the Conflicts pane's edit flow (T071) offers both built-in and custom types when assigning a record's type.
**Test:** Adding a custom type via the web UI, then assigning it to a record through the edit flow, round-trips - the type persists and appears correctly on a later fetch. The 12 built-in types and every existing MCP tool continue to work unchanged.
**Status:** ✅ COMPLETE. Data layer: `supabase/migrations/20260925120000_create_custom_knowledge_types.sql` (`custom_knowledge_types`, unique on `(workspace_id, key)`); `CustomKnowledgeTypeRow` + `list_custom_knowledge_types`/`create_custom_knowledge_type`/`delete_custom_knowledge_type` in `libs/db`. `uv run pytest libs/db -v`: 7 passed (3 new). API layer: `KnowledgeRecord.type` loosened to `str` (one existing test, `test_invalid_type_raises_validation_error`, was replaced with `test_custom_type_strings_are_accepted_by_the_model` - the model no longer rejects unknown type strings itself, by design; `libs/knowledge_model` still 12 passed). `harness-api`'s `GET/POST /taxonomy/types` and `DELETE /taxonomy/types/{key}` built-ins-can't-be-deleted, duplicate-key 409s; `POST /knowledge/{id}/edit` accepts an optional `type`, validated against built-in ∪ custom (400 if unknown). `uv run pytest apps/harness-api -v`: 33 passed, 6 skipped (pre-existing OpenViking-dependent skips, unrelated). Web layer: new `/taxonomy` pane (list built-ins read-only, list/add/delete custom types) added to the shell nav; Conflicts pane's edit form gained a Type `<select>` populated from `/taxonomy/types`. Regenerated `apps/web/lib/harness-api/schema.d.ts` via `scripts/generate-frontend-types.sh`; `npx tsc --noEmit` clean. Verified live in the browser: all 12 built-ins listed correctly; created a custom type ("Meeting Notes" / `meeting_notes`) through the actual form, saw it appear under Custom types, then deleted it through the actual Delete button - round-trips end to end. Did not additionally verify the Conflicts pane's new dropdown live, because OpenViking's dev store already has 200+ accumulated records from today's earlier walkthroughs, past the documented 256-match glob cap - a freshly-seeded record for this check didn't come back from `list_all()`, the same pre-existing issue flagged below, not a T077 defect. The API-level round trip for editing a record's `type` (including via a custom type) is proven directly by the automated test `test_edit_can_change_type_to_a_valid_custom_type`; accepted as sufficient rather than clearing OpenViking's dev data to force a live UI check (user's explicit choice).
...
**Status:** ✅ COMPLETE. Built after the 120s→300s timeout stopgap kept getting hit live, repeatedly, while building T075/T076 - four separate real wedges in one session (`ollama ps` showing a model stuck in "Stopping..." indefinitely, unresponsive even to unrelated requests, not just slow). Ran a second `ollama serve` process bound to a different port (`OLLAMA_HOST=127.0.0.1:11435`), left running in its own terminal tab - completely independent request queue/state from the brew-managed instance on the default port (11434) that OpenViking's container is configured (`infra/openviking-config/ov.conf`, via `host.docker.internal:11434`) to keep warm with its own models. Model files are shared on disk regardless of which port serves them, so nothing needed re-pulling. Pointed our own code at it via `OLLAMA_BASE_URL=http://127.0.0.1:11435`, added to the root `.env.local` (gitignored) with the exact start command documented alongside it.

`uv run pytest libs/llm_router -v` **with OpenViking's container running**: 19 passed in 17.91s - previously this exact command, under the exact same contention conditions, was timing out for 25-30 minutes. Contention is eliminated, not just tolerated by a longer timeout, matching this task's own test criterion. The 300s timeout stopgap stays in place as a safety margin, not removed - it just shouldn't be needed in normal operation anymore. `infra/README.md`'s contention note updated to point at this fix instead of "stop the container first."
...
**Status:** ✅ COMPLETE. First two attempts (before 0008's fix existed) hit the topic-drift gap live, which is what led to building `match_topic`/0008 and T078 mid-task - see those entries above. With both fixes in place, `scripts/e2e_walkthrough_prd_s6_belief_evolution.py` (rewritten to wrap the real `OllamaLLM` in a small logging proxy that prints every `match_topic`/`compare` call and result - the written records' own topic strings aren't enough to tell what happened, since bridging two *different* topic strings is exactly what the fix does) confirmed the mechanism directly: `[match_topic] candidate topic='SSO (Nimbus-...)' vs 243 existing topic(s) -> 'Enterprise SSO'`, then `[compare] ... -> 'superseding'`. The new record's own `supersedes` pointed at an older "Enterprise SSO" fixture from an earlier test run today rather than that run's own meeting-1 record - an artifact of `_write`'s existing `existing[0]` simplification (pipeline.py's own documented note) colliding with this shared dev harness's accumulated duplicate-topic pollution, not a new defect. Either way, PRD §6's actual requirement held: nothing was deleted or silently overwritten - a new record was written, correctly marked as superseding something, with the prior belief still fully present.
...
**Status:** ✅ COMPLETE (with a caveat). `scripts/e2e_walkthrough_prd_s9_contradiction.py` rewritten with the same logging-proxy approach. Two real runs, two different real-model limitations surfaced, neither in the `match_topic`/`compare` mechanism itself: run 1's meeting 2 extracted the project codename itself as its topic ("Project nimbus-...") rather than anything mentioning SSO, so `match_topic` correctly found no bridge (there was no real topical signal in that string to match against); run 2 had both meetings land on the same exact topic ("SSO", no fallback even needed), but meeting 2's own extraction produced a garbled statement (literally the speaker label, "Jordan (Leadership)", not the actual claim about shipping in November) - so `compare()` had nothing meaningful to detect a contradiction in.

Real Ollama walkthroughs are genuinely useful for finding structural gaps (this is exactly how 0007's and 0008's problems were found), but at `llama3.2`'s size on very short, single-line transcripts they're not a reliable pass/fail gate for a specific scenario on demand - extraction quality varies run to run independent of anything built here. The actual thing this task cares about - that a contradicting relationship writes `status: conflicting` (PRD §9's "requires confirmation", not a literal `pending` value - no such `KnowledgeStatus` enum member exists) with both records preserved, no auto-resolution - is already deterministically proven by `test_full_pipeline_writes_a_conflict_record_when_llm_says_contradicting` (T042, `apps/context-agent/tests/test_pipeline.py`), which doesn't depend on a small model's extraction luck. Logged both real runs here as genuine findings rather than re-running indefinitely for a clean real-model pass.

### Real fix — LLM-based topic matching, replacing a tried-and-rejected OpenViking semantic-search fallback
T075's first two runs (real Ollama, real OpenViking) both hit 0007's documented topic-drift gap live: "Enterprise SSO" vs "SSO for Enterprise Customers", then "Enterprise SSO" vs "SSO" - genuinely different LLM-extracted topic strings for the same real-world subject, so `_retrieve` never found the earlier meeting's belief to compare against.

Built and real-tested OpenViking's own semantic search (`search/find`) as the fallback 0007 named as an alternative, then rejected it on real evidence, not theory: OpenViking treats our `.json`-suffixed knowledge records as "code" for summarization purposes, so its VLM-generated file summaries describe generic JSON structure ("a structured data record... no programming-language-specific constructs") rather than actual content - every record gets a near-identical summary, so semantic similarity measured file-shape, not topic. Tried restricting to file-level (`level: [2]`) embeddings and to raw-content embedding (`processing_mode: "vectors_only"`, confirmed via the write response that the VLM step was genuinely skipped) - neither helped; two unrelated old fixture records kept scoring highest for every query regardless of actual relevance. Full detail and the exact scores observed: `docs/decisions/0008-topic-matching-via-llm-not-openviking-semantic-search.md`.

Replaced it with a direct LLM call instead: `llm_router.match_topic()` (new `LLM` interface method; implemented for real in `OllamaLLM`, canned-response support in `FakeLLM`, `NotImplementedError` in `FrontierLLM` matching existing convention), wired into `pipeline._retrieve` as the fallback when the exact-string match misses. Two more things had to be fixed empirically before this was reliable, not assumed up front: asking the model to pick from a list of existing topics in one shot was unreliable (defaulted to "none" even with the correct match in a 3-item list) - switched to asking pairwise, one plain yes/no question per existing topic; and the same yes/no prompt answered differently on separate runs with no other change (Ollama's default sampling isn't deterministic) - pinned `temperature: 0` for this call, added as a new optional parameter on `OllamaLLM._call`.

Also implemented `OllamaLLM.compare()` for real in the same session (was a hard `NotImplementedError` - a separate blocker T075/T076 hit before topic-matching even came up), following the same one-shot-prompt style as `classify()`.

Hit T078's known Ollama/OpenViking contention issue three more times live while testing all of this - consistently reproduced by OpenViking's container being up, not a one-off flake. Further real evidence for T078 (still pending) being the right fix, not the 120s→300s timeout stopgap already in place.

`uv run pytest libs/llm_router apps/context-agent -v`: 19 + 24 passed (with OpenViking stopped, to isolate from the contention issue while iterating). `uv run pytest libs/openviking_client apps/mcp-server apps/harness-api -v` (with OpenViking back up, Ollama untouched by these packages' tests): 61 passed, 6 skipped, no regressions from reverting the semantic-search attempt.
...
**Status: closed, by product-direction decision (2026-09-25), not engineering work.** The user's actual long-term vision for this product isn't "one more org-wide tool competing with `ask-rms` for the same routing decision" - it's a *personal* memory harness first, where a user deliberately shares selected knowledge into a shared team harness at their own discretion, rather than everything landing in one shared pool automatically. Under that framing, direction 2 above (deliberate invocation) is closer to the product's actual shape than a competing-for-automatic-routing problem to solve - a personal tool a person reaches for on purpose isn't fighting an org-wide orchestrator for the same query in the first place. No architecture change follows from this immediately, but it reframes T077's custom-taxonomy work below, and should inform T068-T073's own permission model (already-existing `owner`/`visibility`/`access_level`/`workspace_id` scaffold fields on `KnowledgeRecord`, T039) whenever personal-vs-shared scoping actually gets built. Not scheduled as its own task yet.
