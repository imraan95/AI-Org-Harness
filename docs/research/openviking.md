...
3. **`search/glob` silently caps at 256 matches, with no pagination or truncation flag** — confirmed live (2026-09-25, T061) against this project's real local instance: a recursive `**/*.json` glob under the knowledge root returned exactly `"count": 256` once local test data had accumulated past that point, with matches beyond the cap simply missing from the response - no error, no "has more" indicator. Every glob/grep-based method in `RealOpenVikingClient` (`list_all`, `list_by_type`, `list_conflicts`, and the id-lookup inside `get_knowledge_by_id`/`update_knowledge_status`/`update_knowledge_fields`) is affected once total records exceed 256. Not an issue for local dev (worked around by clearing `infra/openviking-config/data/viking/*/resources/knowledge/` when it fills with test fixtures), but a real, unresolved gap before any production or multi-employee rollout - a genuinely growing knowledge store will exceed 256 records quickly, and list views would silently drop data past that point with no visible error. Needs either a documented pagination mechanism from OpenViking's own API (untested whether one exists), or moving list views off glob/grep entirely toward `search/find`'s semantic search with a high enough result count.
